/*
 * Monitoring API reference
 *
 * Service name  
 *
 * API version: 1.0.0
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package psm_ent

import (
	"encoding/json"
)

// MonitoringSyslogAuditor struct for MonitoringSyslogAuditor
type MonitoringSyslogAuditor struct {
	Config *MonitoringSyslogExportConfig `json:"config,omitempty"`
	// flag to enable sending audit events to syslog.
	Enabled *bool `json:"enabled,omitempty"`
	// audit event export format, SYSLOG_BSD default.
	Format *string `json:"format,omitempty"`
	// export target ip/port/protocol.
	Targets *[]MonitoringExportConfig `json:"targets,omitempty"`
}

// NewMonitoringSyslogAuditor instantiates a new MonitoringSyslogAuditor object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMonitoringSyslogAuditor() *MonitoringSyslogAuditor {
	this := MonitoringSyslogAuditor{}
	var format string = "syslog-bsd"
	this.Format = &format
	return &this
}

// NewMonitoringSyslogAuditorWithDefaults instantiates a new MonitoringSyslogAuditor object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMonitoringSyslogAuditorWithDefaults() *MonitoringSyslogAuditor {
	this := MonitoringSyslogAuditor{}
	var format string = "syslog-bsd"
	this.Format = &format
	return &this
}

// GetConfig returns the Config field value if set, zero value otherwise.
func (o *MonitoringSyslogAuditor) GetConfig() MonitoringSyslogExportConfig {
	if o == nil || o.Config == nil {
		var ret MonitoringSyslogExportConfig
		return ret
	}
	return *o.Config
}

// GetConfigOk returns a tuple with the Config field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MonitoringSyslogAuditor) GetConfigOk() (*MonitoringSyslogExportConfig, bool) {
	if o == nil || o.Config == nil {
		return nil, false
	}
	return o.Config, true
}

// HasConfig returns a boolean if a field has been set.
func (o *MonitoringSyslogAuditor) HasConfig() bool {
	if o != nil && o.Config != nil {
		return true
	}

	return false
}

// SetConfig gets a reference to the given MonitoringSyslogExportConfig and assigns it to the Config field.
func (o *MonitoringSyslogAuditor) SetConfig(v MonitoringSyslogExportConfig) {
	o.Config = &v
}

// GetEnabled returns the Enabled field value if set, zero value otherwise.
func (o *MonitoringSyslogAuditor) GetEnabled() bool {
	if o == nil || o.Enabled == nil {
		var ret bool
		return ret
	}
	return *o.Enabled
}

// GetEnabledOk returns a tuple with the Enabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MonitoringSyslogAuditor) GetEnabledOk() (*bool, bool) {
	if o == nil || o.Enabled == nil {
		return nil, false
	}
	return o.Enabled, true
}

// HasEnabled returns a boolean if a field has been set.
func (o *MonitoringSyslogAuditor) HasEnabled() bool {
	if o != nil && o.Enabled != nil {
		return true
	}

	return false
}

// SetEnabled gets a reference to the given bool and assigns it to the Enabled field.
func (o *MonitoringSyslogAuditor) SetEnabled(v bool) {
	o.Enabled = &v
}

// GetFormat returns the Format field value if set, zero value otherwise.
func (o *MonitoringSyslogAuditor) GetFormat() string {
	if o == nil || o.Format == nil {
		var ret string
		return ret
	}
	return *o.Format
}

// GetFormatOk returns a tuple with the Format field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MonitoringSyslogAuditor) GetFormatOk() (*string, bool) {
	if o == nil || o.Format == nil {
		return nil, false
	}
	return o.Format, true
}

// HasFormat returns a boolean if a field has been set.
func (o *MonitoringSyslogAuditor) HasFormat() bool {
	if o != nil && o.Format != nil {
		return true
	}

	return false
}

// SetFormat gets a reference to the given string and assigns it to the Format field.
func (o *MonitoringSyslogAuditor) SetFormat(v string) {
	o.Format = &v
}

// GetTargets returns the Targets field value if set, zero value otherwise.
func (o *MonitoringSyslogAuditor) GetTargets() []MonitoringExportConfig {
	if o == nil || o.Targets == nil {
		var ret []MonitoringExportConfig
		return ret
	}
	return *o.Targets
}

// GetTargetsOk returns a tuple with the Targets field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MonitoringSyslogAuditor) GetTargetsOk() (*[]MonitoringExportConfig, bool) {
	if o == nil || o.Targets == nil {
		return nil, false
	}
	return o.Targets, true
}

// HasTargets returns a boolean if a field has been set.
func (o *MonitoringSyslogAuditor) HasTargets() bool {
	if o != nil && o.Targets != nil {
		return true
	}

	return false
}

// SetTargets gets a reference to the given []MonitoringExportConfig and assigns it to the Targets field.
func (o *MonitoringSyslogAuditor) SetTargets(v []MonitoringExportConfig) {
	o.Targets = &v
}

func (o MonitoringSyslogAuditor) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Config != nil {
		toSerialize["config"] = o.Config
	}
	if o.Enabled != nil {
		toSerialize["enabled"] = o.Enabled
	}
	if o.Format != nil {
		toSerialize["format"] = o.Format
	}
	if o.Targets != nil {
		toSerialize["targets"] = o.Targets
	}
	return json.Marshal(toSerialize)
}

type NullableMonitoringSyslogAuditor struct {
	value *MonitoringSyslogAuditor
	isSet bool
}

func (v NullableMonitoringSyslogAuditor) Get() *MonitoringSyslogAuditor {
	return v.value
}

func (v *NullableMonitoringSyslogAuditor) Set(val *MonitoringSyslogAuditor) {
	v.value = val
	v.isSet = true
}

func (v NullableMonitoringSyslogAuditor) IsSet() bool {
	return v.isSet
}

func (v *NullableMonitoringSyslogAuditor) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMonitoringSyslogAuditor(val *MonitoringSyslogAuditor) *NullableMonitoringSyslogAuditor {
	return &NullableMonitoringSyslogAuditor{value: val, isSet: true}
}

func (v NullableMonitoringSyslogAuditor) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMonitoringSyslogAuditor) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


