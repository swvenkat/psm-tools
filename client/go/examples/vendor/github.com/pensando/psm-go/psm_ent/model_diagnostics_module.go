/*
 * Diagnostics API reference
 *
 * Service name  
 *
 * API version: 1.0.0
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package psm_ent

import (
	"encoding/json"
)

// DiagnosticsModule Module contains configuration to collect diagnostics information about a process and collected diagnostics.
type DiagnosticsModule struct {
	ApiVersion *string `json:"api-version,omitempty"`
	Kind *string `json:"kind,omitempty"`
	Meta *ApiObjectMeta `json:"meta,omitempty"`
	Spec *DiagnosticsModuleSpec `json:"spec,omitempty"`
	Status *DiagnosticsModuleStatus `json:"status,omitempty"`
}

// NewDiagnosticsModule instantiates a new DiagnosticsModule object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDiagnosticsModule() *DiagnosticsModule {
	this := DiagnosticsModule{}
	return &this
}

// NewDiagnosticsModuleWithDefaults instantiates a new DiagnosticsModule object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDiagnosticsModuleWithDefaults() *DiagnosticsModule {
	this := DiagnosticsModule{}
	return &this
}

// GetApiVersion returns the ApiVersion field value if set, zero value otherwise.
func (o *DiagnosticsModule) GetApiVersion() string {
	if o == nil || o.ApiVersion == nil {
		var ret string
		return ret
	}
	return *o.ApiVersion
}

// GetApiVersionOk returns a tuple with the ApiVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DiagnosticsModule) GetApiVersionOk() (*string, bool) {
	if o == nil || o.ApiVersion == nil {
		return nil, false
	}
	return o.ApiVersion, true
}

// HasApiVersion returns a boolean if a field has been set.
func (o *DiagnosticsModule) HasApiVersion() bool {
	if o != nil && o.ApiVersion != nil {
		return true
	}

	return false
}

// SetApiVersion gets a reference to the given string and assigns it to the ApiVersion field.
func (o *DiagnosticsModule) SetApiVersion(v string) {
	o.ApiVersion = &v
}

// GetKind returns the Kind field value if set, zero value otherwise.
func (o *DiagnosticsModule) GetKind() string {
	if o == nil || o.Kind == nil {
		var ret string
		return ret
	}
	return *o.Kind
}

// GetKindOk returns a tuple with the Kind field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DiagnosticsModule) GetKindOk() (*string, bool) {
	if o == nil || o.Kind == nil {
		return nil, false
	}
	return o.Kind, true
}

// HasKind returns a boolean if a field has been set.
func (o *DiagnosticsModule) HasKind() bool {
	if o != nil && o.Kind != nil {
		return true
	}

	return false
}

// SetKind gets a reference to the given string and assigns it to the Kind field.
func (o *DiagnosticsModule) SetKind(v string) {
	o.Kind = &v
}

// GetMeta returns the Meta field value if set, zero value otherwise.
func (o *DiagnosticsModule) GetMeta() ApiObjectMeta {
	if o == nil || o.Meta == nil {
		var ret ApiObjectMeta
		return ret
	}
	return *o.Meta
}

// GetMetaOk returns a tuple with the Meta field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DiagnosticsModule) GetMetaOk() (*ApiObjectMeta, bool) {
	if o == nil || o.Meta == nil {
		return nil, false
	}
	return o.Meta, true
}

// HasMeta returns a boolean if a field has been set.
func (o *DiagnosticsModule) HasMeta() bool {
	if o != nil && o.Meta != nil {
		return true
	}

	return false
}

// SetMeta gets a reference to the given ApiObjectMeta and assigns it to the Meta field.
func (o *DiagnosticsModule) SetMeta(v ApiObjectMeta) {
	o.Meta = &v
}

// GetSpec returns the Spec field value if set, zero value otherwise.
func (o *DiagnosticsModule) GetSpec() DiagnosticsModuleSpec {
	if o == nil || o.Spec == nil {
		var ret DiagnosticsModuleSpec
		return ret
	}
	return *o.Spec
}

// GetSpecOk returns a tuple with the Spec field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DiagnosticsModule) GetSpecOk() (*DiagnosticsModuleSpec, bool) {
	if o == nil || o.Spec == nil {
		return nil, false
	}
	return o.Spec, true
}

// HasSpec returns a boolean if a field has been set.
func (o *DiagnosticsModule) HasSpec() bool {
	if o != nil && o.Spec != nil {
		return true
	}

	return false
}

// SetSpec gets a reference to the given DiagnosticsModuleSpec and assigns it to the Spec field.
func (o *DiagnosticsModule) SetSpec(v DiagnosticsModuleSpec) {
	o.Spec = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *DiagnosticsModule) GetStatus() DiagnosticsModuleStatus {
	if o == nil || o.Status == nil {
		var ret DiagnosticsModuleStatus
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DiagnosticsModule) GetStatusOk() (*DiagnosticsModuleStatus, bool) {
	if o == nil || o.Status == nil {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *DiagnosticsModule) HasStatus() bool {
	if o != nil && o.Status != nil {
		return true
	}

	return false
}

// SetStatus gets a reference to the given DiagnosticsModuleStatus and assigns it to the Status field.
func (o *DiagnosticsModule) SetStatus(v DiagnosticsModuleStatus) {
	o.Status = &v
}

func (o DiagnosticsModule) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.ApiVersion != nil {
		toSerialize["api-version"] = o.ApiVersion
	}
	if o.Kind != nil {
		toSerialize["kind"] = o.Kind
	}
	if o.Meta != nil {
		toSerialize["meta"] = o.Meta
	}
	if o.Spec != nil {
		toSerialize["spec"] = o.Spec
	}
	if o.Status != nil {
		toSerialize["status"] = o.Status
	}
	return json.Marshal(toSerialize)
}

type NullableDiagnosticsModule struct {
	value *DiagnosticsModule
	isSet bool
}

func (v NullableDiagnosticsModule) Get() *DiagnosticsModule {
	return v.value
}

func (v *NullableDiagnosticsModule) Set(val *DiagnosticsModule) {
	v.value = val
	v.isSet = true
}

func (v NullableDiagnosticsModule) IsSet() bool {
	return v.isSet
}

func (v *NullableDiagnosticsModule) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDiagnosticsModule(val *DiagnosticsModule) *NullableDiagnosticsModule {
	return &NullableDiagnosticsModule{value: val, isSet: true}
}

func (v NullableDiagnosticsModule) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDiagnosticsModule) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


